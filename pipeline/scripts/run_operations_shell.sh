...
run_sar_feature_smoke() {
  ...
  if [[ -f "$PROJECT_ROOT/pipeline/config/geopolitical_events.json" ]]; then
    geo_filter_arg=(--geopolitical-event-filter "$PROJECT_ROOT/pipeline/config/geopolitical_events.json")
  fi
...
run_download_ais_marine_cadastre() {
  ...
  if ! run_cmd uv run python -m pipeline.src.ingest.marine_cadastre \
      --year "$year" \
      --db "$db_path" \
...
run_fetch_aishub() {
  ...
  if ! run_cmd uv run python -m pipeline.src.ingest.aishub \
      --username "$username" \
      --db "$db_path" \
      "${bbox_args[@]+"${bbox_args[@]}"}"; then
    echo "Result: FAILED"
...
run_fetch_aisstream() {
  ...
  if ! run_cmd uv run python -m pipeline.src.ingest.ais_stream \
      --db "$db_path" \
      --duration "$duration" \
      "${bbox_args[@]+"${bbox_args[@]}"}"; then
    echo "Result: FAILED"
...
" 2>/dev/null)"
  ...
  if ! prompt_yes_no "Run feature matrix + scoring + Precision@50?" "false"; then
    ...
    echo "  Add to crontab:  */30 * * * * cd $PROJECT_ROOT && uv run python -m pipeline.src.ingest.ais_stream --db $db_path --duration 300 ${bbox_args[*]+"${bbox_args[*]}"}"
    return
  fi
